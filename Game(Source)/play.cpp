...
void Play::Run ()
{
    ...
    goTank = InitTank (1, 8);
    goTank->SetPosition (LumenAusf::vec2 (0.f, -0.5f));

    goTank2 = InitTank (9, 16);
    goTank2->SetPosition (LumenAusf::vec2 (0.f, 0.5f));
...
Tank* Play::InitTank (int AtlasStart, int AtlasEnd)
{
    ...
    go->transform.scale = LumenAusf::mat2x3::scale (0.5f);
...
Tank* Play::CreateTank (std::string TrianglesPath, LumenAusf::Texture* texture, int TrianglesCount, int AtlasStart, int AtlasEnd)
{
    ...
    auto go = new Tank (*a, triangles);
    go->SetAtlas (texture, LumenAusf::vec2 (8, 4), LumenAusf::vec2 (AtlasStart, AtlasEnd));
    return go;
}
